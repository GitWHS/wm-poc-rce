(function(){
  document.title='P80MAIN:'+location.host;
  window.__p=function(r){
    ...
    document.title='P80MCB:'+(r&&r.callId)+':'+location.host+':'+(r&&r.code);
  };
  ...
  var i=0;
  function n(){
    if(i>=q.length){document.title='P80MAIN-DONE:'+location.host;return}
    var x=q[i++];
    c(x[0],x[1],x[2],x[3]);
    setTimeout(n,3500);
  }
