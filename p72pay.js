(function(){
  document.title='P72PAY:'+location.host;
  window.__p=function(r){
    ...
    document.title='P72PCB:'+(r&&r.callId)+':'+(r&&r.code)+':'+String(r&&r.message||JSON.stringify(r&&r.data||{})).slice(0,72);
  };
  ...
  var q=[
    ...
    ['I3','IdeaWebViewPlugin','DownloadIdeaFile',{url:zip,title:'p72',code:'123456789'}],
  ...
  var i=0;
  function n(){
    if(i>=q.length){document.title='P72PAY-DONE';return}
    var x=q[i++];
    c(x[0],x[1],x[2],x[3]);
    setTimeout(n,1800);
  }
