(function(){
  document.title='P127TB:'+location.host;
  window.__p=function(r){
    try{r=typeof r==='string'?JSON.parse(r):r}catch(e){}
    document.title='P127CB:'+(r&&r.callId)+':'+location.host+':'+(r&&r.code)+':'+String(r&&r.message||JSON.stringify(r&&r.data||{})).slice(0,80);
  };
  function c(id,mod,m,p){
    try{
      ...
      document.title='P127TB-NOJS:'+location.host;
    }
  }
  if(!window.JSCallJavaMgr){document.title='P127TB-NOMGR:'+location.host;return}
  ...
  var i=0;
  function n(){
    if(i>=q.length){document.title='P127TB-DONE:'+location.host;return}
    var x=q[i++];
    document.title='P127-RUN:'+x[0];
    c(x[0],x[1],x[2],x[3]);
    setTimeout(n, 4000);
  }
  setTimeout(n, 800);
