(function(){
  document.title='P70JS:'+location.host;
  window.__p=function(r){
    ...
    document.title='P70CB:'+(r&&r.callId)+':'+(r&&r.code)+':'+String(r&&r.message||JSON.stringify(r&&r.data||{})).slice(0,56);
  };
  ...
  var q=[
    ...
    ['L1','CommonPlugin','LoadUrl',{url:official}],
    ['N1','CommonPlugin','OpenNewPage',{url:pay,scheme:pay}],
    ['N2','CommonPlugin','OpenNewPage',{scheme:pay}],
  ...
  var i=0;
  function n(){
    if(i>=q.length){document.title='P70-DONE';return}
    var x=q[i++];
    c(x[0],x[1],x[2],x[3]);
    setTimeout(n,900);
  }
