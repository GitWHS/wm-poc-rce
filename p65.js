(function(){
  document.title='P65JS:'+location.host;
  window.__p=function(r){
    ...
    document.title='P65CB:'+(r&&r.callId)+':'+(r&&r.code)+':'+String(r&&r.message||JSON.stringify(r&&r.data||{})).slice(0,56);
  };
  ...
  var q=[
    ...
    ['U1','CommonPlugin','UnzipFile',{url:zip}],
    ['U2','CommonPlugin','DownloadAndUnzip',{url:zip}],
    ['U3','CommonPlugin','UnzipResource',{url:zip}],
    ['H1','CommonPlugin','GetPerformance',{}]
  ];
  var i=0;
  function n(){
    if(i>=q.length){document.title='P65-DONE';return}
    var x=q[i++];
    c(x[0],x[1],x[2],x[3]);
    setTimeout(n,1100);
  }
